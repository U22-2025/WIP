...
fn test_location_response_creation() {
    let response = LocationResponse::success(1, 123456);
    
    assert_eq!(response.packet_id, 1);
    assert_eq!(response.area_code, 123456);
}
...
fn test_query_request_creation() {
    let request = QueryRequest::create_query_request("123456", 1, false, false, false, false, false, 0, 1);
...
fn test_query_request_serialization() {
    let request = QueryRequest::create_query_request("123456", 1, true, true, false, false, false, 0, 1);
...
fn test_edge_case_coordinates() {
    ...
    for (lat, lon) in test_cases {
        let request = LocationRequest::create_coordinate_lookup(lat, lon, 1, false, false, false, false, false, 0, 1);
