...
wt ^
  new-tab cmd /k "cd /d %CURRENT_DIR% && timeout /t 1 /nobreak > nul && python python/launch_server.py --weather" ^
  ; split-pane -V cmd /k "cd /d %CURRENT_DIR%  && timeout /t 2 /nobreak > nul && python python/launch_server.py --query" ^
  ; focus-pane -t 0 ^
  ; split-pane -H cmd /k "cd /d %CURRENT_DIR% && timeout /t 3 /nobreak > nul && python python/launch_server.py --location" ^
  ; focus-pane -t 1 ^
  ; split-pane -H cmd /k "cd /d %CURRENT_DIR% && timeout /t 4 /nobreak > nul && python python/launch_server.py --report" ^
  ; split-pane -V cmd /k "cd /d %CURRENT_DIR% && timeout /t 5 /nobreak > nul && python python/application/map/start_fastapi_server.py" ^
