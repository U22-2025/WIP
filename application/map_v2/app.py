...
def disaster_info():
    ...
    client.set_coordinates(lat, lng)
    result = client.get_weather(weather=False, temperature=False, precipitation_prob=False, alert=False, disaster=True)

    if not result:
        return jsonify({"status": "error", "message": "災害情報の取得に失敗しました"}), 500
