...
from WIPCommonPy.packet.core.extended_field import reload_extended_spec
...
def traditional_usage_example():
    ...
    print(
        f"  Flags: {request.weather_flag}, {request.temperature_flag}, {request.pop_flag}"
    )
...
def response_processing_example():
    ...
    if hasattr(sample_response, "ex_field") and sample_response.ex_field:
        alert = (
            sample_response.ex_field.alert
            if hasattr(sample_response.ex_field, "alert")
            else []
        )
        print(f"  警報: {alert}")
...
def client_integration_example():
    ...
    sapporo_request = create_query_request_easily(
        43.0642, 141.3469, {"weather": True, "temperature": True, "alerts": True}
    )
...
def compatibility_test():
    ...
    print(
        f"  Coordinates: {traditional_req.ex_field.latitude}, {traditional_req.ex_field.longitude}"
    )
