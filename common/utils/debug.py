...
def show_help():
    ...
    print(
        "  python debug.py full --no-performance    # パフォーマンステストなしでフルテスト"
    )
    print(
        "  python debug.py performance --sizes 100 500 1000  # カスタムサイズでパフォーマンステスト"
    )
    ...
    try:
        ...
        if mode != "full":
            ...
            output_file = kwargs.get(
                "output", os.path.join(reports_dir, f"{mode}_debug_report.json")
            )
...
def main():
    ...
    parser.add_argument(
        "--no-performance", action="store_true", help="パフォーマンステストをスキップ"
    )
    ...
    parser.add_argument("--help", "-h", action="store_true", help="ヘルプを表示")
