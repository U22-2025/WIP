...
#### 地図設定
`static/js/weather-app.js` で地図の初期設定を変更できます。デフォルトでは Google の航空写真タイルを使用し、日本国内のみ表示されるよう `maxBounds` が設定されています。

地図表示を軽くするためのポイント:

- 表示範囲を日本国内に限定することで不要なタイル読み込みを削減
- `maxZoom` を必要最小限に設定し、ズームレベルを抑える
- 可能であればブラウザのキャッシュを活用し、タイルの再読み込みを減らす
