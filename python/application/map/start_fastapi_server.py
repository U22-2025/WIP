...
if __name__ == "__main__":
    ...
    log_host = config_loader.get(
        "logging", "log_redis_host", os.getenv("LOG_REDIS_HOST", "localhost")
    )
    log_port = config_loader.get(
        "logging", "log_redis_port", os.getenv("LOG_REDIS_PORT", "6380")
    )
    log_db = config_loader.get(
        "logging", "log_redis_db", os.getenv("LOG_REDIS_DB", "0")
    )
    ...
    if sys.platform.startswith("win") and workers > 1:
        print(
            "Warning: Windows環境ではworkers>1はサポートされていません。workers=1に変更します"
        )
        workers = 1
