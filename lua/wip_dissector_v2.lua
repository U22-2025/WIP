...
-- Generated from python/common/packet/format_spec definitions

-- プロトコル定義
local wip_proto = Proto("WIPv1", "Weather Transmission Protocol")
...
local f_request_auth = ProtoField.bool("wip.request_auth", "Request Auth", 8, nil, 0x02)
local f_response_auth = ProtoField.bool("wip.response_auth", "Response Auth", 8, nil, 0x04)
local f_day = ProtoField.uint8("wip.day", "Days", base.DEC, nil, 0x38)
local f_reserved = ProtoField.uint8("wip.reserved", "Reserved", base.HEX, nil, 0xC0)
...
local f_weather_code = ProtoField.uint16("wip.weather_code", "Weather Code", base.DEC)
local f_temperature = ProtoField.uint8("wip.temperature", "Temperature", base.DEC)
local f_pops = ProtoField.uint8("wip.pops", "precipitation_prob Probability (%)", base.DEC)
...
wip_proto.fields = {
    ...
    f_ex_flag, f_request_auth, f_response_auth, f_day, f_reserved,
...
function wip_proto.dissector(buffer, pinfo, tree)
    ...
    local request_auth = extract_bits_le(buffer(0, 4), 25, 1)
    local response_auth = extract_bits_le(buffer(0, 4), 26, 1)
    local day = extract_bits_le(buffer(0, 4), 27, 3)
    local reserved = extract_bits_le(buffer(0, 4), 30, 2)
    ...
    flag_tree:add_le(f_request_auth, buffer(3, 1)):append_text(" (" .. request_auth .. ")")
    flag_tree:add_le(f_response_auth, buffer(3, 1)):append_text(" (" .. response_auth .. ")")
