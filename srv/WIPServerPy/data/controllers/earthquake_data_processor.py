...
class EarthquakeDataProcessor:
    ...
    def get_earthquake_xml_list(self) -> List[str]:
        ...
        Returns:
            XMLファイルURLのリスト
        """
        return self.xml_processor.get_feed_entry_urls("https://www.data.jma.go.jp/developer/xml/feed/eqvol.xml")
    
    def get_earthquake_info(self, url_list: List[str], output_json_path: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            if '地震情報(' in earthquake_info and ')' in earthquake_info:
                ...
                if start_idx != -1 and end_idx != -1:
                    content = earthquake_info[start_idx+4:end_idx]  # "地震情報("の後から")"の前まで
    ...
    def format_to_alert_style(self, area_kind_mapping: Dict[str, List[str]], area_report_times: Dict[str, str] = None, area_codes_data: Dict = None) -> Dict[str, Any]:
        ...
        for area_code, earthquake_list in area_kind_mapping.items():
            ...
            if len(earthquake_list) > 1:
                consolidated_earthquake = self._consolidate_earthquakes_for_area(earthquake_list)
